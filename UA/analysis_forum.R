...
#### load data ####
data <- read.csv("data/Kestrel_forum.csv", skip = 9, header = T)
data <- data[-1, ]

protocol <- read.csv("data/measurement_protocol_route_forum.csv",  skip = 6, header = T)
...
# write.csv(filtered_data, "data/filetered_data_forum.csv", row.names = F)
...
filtered_data<- filtered_data %>% filter(site.name != "rudower south shelter")
filtered_data<- filtered_data %>% filter(site.name != "rudower north shelter")
