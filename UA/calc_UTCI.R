calc_utci = function(ta = NULL, tmrt = NULL, hur = NULL, ws = NULL ) {
  ...
  args = c("ta","tmrt","hur","ws")
  check.passed = args %in% passed
  
  if(length(passed[!check.passed]) > 0) stop(paste(args[!check.passed], collapse = ", ")," not defined!")
  
  check.length = unique(length(ta),length(tmrt),length(hur),length(ws))
  if (length(check.length) != 1) {
    stop(paste("input data have different lengths!"))
  }
  ...
  ehPa = es * hur/100.00000000
  
  Pa = ehPa/10.0; #!~ use vapour pressure in kPa
  va = (log(10/0.01)/log(1.77/0.01))*ws
